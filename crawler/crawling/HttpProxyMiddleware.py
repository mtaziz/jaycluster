...
class HttpProxyMiddleware(object):
    ...
    def from_crawler(cls, crawler):
        ...
        my_name = "%s_%s" % (crawler.spidercls, get_raspberrypi_ip_address())
    ...
    def reset_proxyes(self):
        ...
        """
        self.logger.info("reset proxyes to valid")
        for p in self.proxyes:
    ...
    def inc_proxy_index(self):
        ...
        if self.len_valid_proxy() < self.extend_proxy_threshold: # 代理数量仍然不足, 抓取新的代理
            self.logger.info("valid proxy < threshold: %d/%d" % (self.len_valid_proxy(), self.extend_proxy_threshold))
            self.fetch_new_proxyes()

            self.logger.info("now using new proxy: %s" % self.proxyes[self.proxy_index]["proxy"])
    ...
    def dump_valid_proxy(self):
        ...
        if self.dump_count_threshold <= 0:
            return
            self.logger.info("dumping proxyes to file")
    ...
    def process_request(self, request, spider):
        ...
        if self.proxy_index > 0  and datetime.now() > (self.last_no_proxy_time + timedelta(minutes=self.recover_interval)):
            self.logger.info("After %d minutes later, recover from using proxy" % self.recover_interval)
            self.last_no_proxy_time = datetime.now()
            self.proxy_index = 0
