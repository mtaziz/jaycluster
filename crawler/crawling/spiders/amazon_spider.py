...
class AmazonSpider(JayClusterSpider):
    ...
    def parse(self, response):
        result_count = re_search(r'of(.*)results', ''.join(response.xpath('//*[@id="s-result-count"]/text()').extract()).strip())
        self.job_status_monitor.set_total_pages(response.meta['crawlid'], response.meta['spiderid'], response.meta['appid'], result_count)
    ...
    def parse_item_update(self, response):
        ...
        if len(robot_checks) > 0:
            ...
            if item['meta']['workers'][self.worker_id] >= 3:
                self.job_status_monitor.inc_drop_pages(
                    crawlid=response.meta['crawlid'],
                    spiderid=response.meta['spiderid'],
                    appid=response.meta['appid']
                )
                ...
                self.job_status_monitor.inc_banned_pages(
                    crawlid=response.meta['crawlid'],
                    spiderid=response.meta['spiderid'],
                    appid=response.meta['appid']
                )
